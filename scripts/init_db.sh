#!/usr/bin/env bash
# Initialise a postgres db on localhost for testing
...
# Check if a custom database name has been set, otherwise default to 'dtbh'
DB_NAME="${POSTGRES_DB:=dtbh}"
# Check if a custom port has been set, otherwise default to '5432'
DB_PORT="${POSTGRES_PORT:=5432}"

# Only run new container if SKIP_DOCKER is false
if [[ -z "${SKIP_DOCKER}" ]]
then
...
>&2 echo "Running migrations on ${DB_NAME}"
